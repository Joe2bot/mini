...
public class RunService {
    private final RunRepository runs; private final TraceRepository traces; private final AgentService agents; private final ObjectMapper mapper;
    public RunService(RunRepository runs, TraceRepository traces, AgentService agents, ObjectMapper mapper){this.runs=runs;this.traces=traces;this.agents=agents;this.mapper=mapper;}
    @Transactional public RunCreated createAgentRun(UUID agentId, String input) {
        ...
        ObjectNode snapshot=mapper.createObjectNode().set("agent",agentSnapshot);
